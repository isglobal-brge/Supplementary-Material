...
#Remove the CpG sites from the EWAS catalog that are not in TruD
for (i in 1:3){
  catalogues[[i]] <- catalogues[[i]][which(catalogues[[i]]$cpg%in%truD[[i]]$CpG),]
}
...
venn <- function(substance){
  ...
  df_sig <- df_truD[which(df_truD$adj.P.Val<0.05),]
  cpg_tru <- df_sig$CpG
  
  #Create Venn diagram
  venn.diagram(
    x = list(df$CpG, cpg_tru),
    category.names = c("EWAS catalog","TruDiagnostic"),
    filename = paste0("EWAS_catalog/venn_diagram_EWAScatalog_",substance,"_sigTruD.png"),
    output=TRUE,
...
#Compare top new CpG sites
df_truD <- truD[[1]]
df_sig <- df_truD[which(df_truD$adj.P.Val<0.05),]
cpg_tru <- df_sig$CpG

cpg_cat <- unique(catalogues[[1]]$cpg)

cpgs_new <- setdiff(cpg_tru,cpg_cat)

head(df_truD[cpgs_new,c(7,8,10:14)])
