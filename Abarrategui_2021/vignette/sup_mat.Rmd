...
This manual describes the `epimutacions` package tools to identify epivariants using multiple outlier detection  approaches. Also, includes functions to plot and annotate the epimutations. 
...
If you want to combine data from different studies, normalization is highly recommended. The preprocessing step removes the unwanted variation caused by the batch effect. In addition, it converts raw  microarray intensities to `GenomicRatioSet` (necessary input class for `epimutations()` function). Finally, the case and control samples are introduced separately in `epimutations()` function. 

The figure \ref{fig:workflow} shows the workflow related to the data types, normalization and inputs of `epimutations()` function.  
...
The `epimutacions` package is available on GitHub and can be installed by executing the following comand: 
```{r, eval=FALSE}
...
\captionof{table}{Preprocessing methods unique parameters}
...
The `epi_parameters()` function  is  useful to set the individual parameters for each approach. The arguments are described in the table \ref{tab:epi_unique_parameter}: 

\captionof{table}{epimutation function approaches  unique parameters}
...
```{r plot_default, fig.cap="beta values of case sample againts control samples"}
...
```{r, plot_mvo_genes_annot, fig.cap="beta values of case sample againts control samples including UCSC gene annotation"}
...
We evaluated the performance of the method using True Positive Rate(TPR), False Positive Rate (FPR) and accuracy. We use the TPR to measure the proportion of detected epivariations by the `epimutations` approach  present in the validated (table \@ref(tab:tableEpiValidated)) by [@garg2020survey]. FPR to calculate the identified epimutations outside the once found in [@garg2020survey], whether validated or not. The accuracy measures the closeness of the detected epimutation to the validated regions. 
...
sample against control samples. We observed that in both cohorts and for every approach the FPR value is minimum $<0.01\%$. 
